...
file_path ="./article/news-contents.csv"
df = pd.read_csv(file_path)

# 한글 키워드 추출 함수

def extract_keywords(texts, all_text):
    Nouns=[]
    all_text+=texts
    # words = re.findall(r'[가-힣]{2,}', all_text)
    all_text= re.sub('[기자|기사|문의|카톡|라인|연합뉴스|뉴스|제보]','', all_text)
    # words=okt.pos(all_text)
    words=komoran.pos(all_text)
    print(type(words))
    print(words)
    for i in words:
        if(i[1]=='NNG' or i[1]=='NNP' and (len(i[0])>=2)):
            # if(i[i]!=("기자" and "기사" and "문의" and "카톡" and "라인") ):
...
# print(all_content)
print(all_content)
keywords= extract_keywords(all_content,all_text)
